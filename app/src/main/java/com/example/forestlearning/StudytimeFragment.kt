...
class StudytimeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding?.addButtonStudytime?.setOnClickListener {
            findNavController().navigate(R.id.action_studytimeFragment_to_subjectadderFragment)
        }

        /**var subject = Subjects()

        adapter = StudytimeAdapter(subject)
        binding?.subjectScrollView?.layoutManager = LinearLayoutManager(context)
        binding?.subjectScrollView?.adapter = adapter**/
    }
